 ...
 * limitations under the License.
  ...
  small changes
  - added subscriber to receiver/signal
  - added callback for this message to stop sampler in case the victim search
    and vector following are active

  Changes added by Nicholas and Maximilian
...
TrajectorySamplerNode::TrajectorySamplerNode(const ros::NodeHandle& nh,
  ...
  //subscriber for receiver/signal
  beacon_status_sub = nh_.subscribe("receiver/signal", 100, &TrajectorySamplerNode::onBeaconMsg, this);
...
//Function to start or stop the publishing of the commands if victim search is activ
void TrajectorySamplerNode::onBeaconMsg(const beacon::receiver beacon_msg){

    if (beacon_msg.distance < 3500 and beacon_msg.distance > 0) {
      std::cout << "[ TrajectorySamplerNode ] Stopping trajectory sampling" << std::endl;
      publish_timer_.stop();
    } else {
