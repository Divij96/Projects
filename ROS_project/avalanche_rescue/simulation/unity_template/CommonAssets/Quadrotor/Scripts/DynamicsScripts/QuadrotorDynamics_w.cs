...
public class QuadrotorDynamics_w : MonoBehaviour {
	...
	public float thrust_coeff = (float)0.001;
	public float drag_coeff = (float)0.00001;
	// public float c_f = (float)0.001;
	// public float c_d = (float)0.00001;
	public float arm_length = (float)0.3;
	public float mass = (float) 1;
	public float Inertia_xx = (float)0.1;
	public float Inertia_yy = (float)0.1;
	public float Inertia_zz = (float)0.2;
	public float drag = (float)0;
	public float angular_drag = (float)0.05;

	float[] w_ = { 0, 0, 0, 0 };
	float c_f = (float)0.001;
	float c_d = (float)0.00001;
	...
	void Start () {
		...
		float c_f = thrust_coeff;
		float c_d = drag_coeff;
		rb = GetComponent<Rigidbody> ();
		rb.mass = mass;
		rb.inertiaTensor = new Vector3 (Inertia_xx,Inertia_yy,Inertia_zz);
		rb.drag = drag;
