...
// ==========================================
//
//  Description: Based on the script given in 
//  various labs. Added a marker message for rviz.
//  The marker takes the quadrotor mesh given in
//  Lab2 for visualization purposes.
...
class StateServer
{
  public:
    ...
    void PublishStateStream() {
      ...
      try {
        ...
        state_marker.ns = "Drone";
        state_marker.id = 42;
        state_marker.type = visualization_msgs::Marker::MESH_RESOURCE;
        state_marker.action = visualization_msgs::Marker::ADD;
        state_marker.mesh_resource = "package://unity_bridge/mesh/quadrotor.dae";
        state_marker.pose.position.x = begin[0];
        state_marker.pose.position.y = begin[2];
        state_marker.pose.position.z = begin[1];
        state_marker.pose.orientation.x = -begin[3];
        state_marker.pose.orientation.y = -begin[5];
        state_marker.pose.orientation.z = -begin[4];
        state_marker.pose.orientation.w = begin[6];
        state_marker.scale.x = 20;
        state_marker.scale.y = 20;
        state_marker.scale.z = 20;
        state_marker.color.b = 1;
        state_marker.color.g = .52;
        state_marker.color.r = .25;
        state_marker.color.a = 1.0;
        state_marker.lifetime = ros::Duration(1.0);

        marker_pub.publish(state_marker);
  ...
  void InitializePublishers() {
    ...
    marker_pub = nh.advertise<visualization_msgs::Marker>("/current_state_marker", 0 );
  }
